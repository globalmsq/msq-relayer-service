...
describe('SqsAdapter', () => {
  ...
  describe('initialization', () => {
    it('should throw error if SQS_QUEUE_URL is not configured', () => {
      ...
      // Act & Assert
      expect(() => new SqsAdapter(mockConfigService as unknown as ConfigService)).toThrow(
    ...
    it('should initialize with LocalStack credentials when endpoint is provided', () => {
      ...
      // Act
      const localAdapter = new SqsAdapter(mockConfigService as unknown as ConfigService);
    ...
    it('should initialize without explicit credentials for production (IAM Role)', () => {
      ...
      const mockConfigService = createMockConfigService({
        ...
        'sqs.queueUrl': 'https://sqs.ap-northeast-2.amazonaws.com/123456789/relay-transactions',
        'sqs.region': 'ap-northeast-2',
      });

      // Act
      const prodAdapter = new SqsAdapter(mockConfigService as unknown as ConfigService);
  ...
  describe('sendMessage', () => {
    ...
    it('should throw error when SQS send fails', async () => {
      ...
      // Act & Assert
      await expect(adapter.sendMessage(messageBody)).rejects.toThrow('SQS service unavailable');
    });
